...
const computeItemBaseRotation = (x, y, sizeX, sizeY, segments) => {
    const rotYUnit = (360 / segments) / 2;
    const rotXUnit = (360 / segments) / 2;

    const rotateY = rotYUnit * (x + (sizeX - 1) / 2);
    const rotateX = rotXUnit * (y - (sizeY - 1) / 2);
...
function buildItems(pool, seg) {
    ...
    return coords.map((c, i) => {
        ...
        return { ...c, src: img.src, alt: img.alt, id: img.id };
    });
}
...
export default function SphereGallery({
    ...
    maxRadius = 800,
    padFactor = 0.1,
    overlayBlurColor = 'rgba(0,0,0,0.6)',
    grayscale = false,
    ...
    const openItemFromElement = el => {
        ...
        if (rawId) {
            ...
            overlay.style.pointerEvents = 'auto'; // Ensure clicks are captured

            // Add click listener for navigation
            overlay.onclick = (e) => {
                e.stopPropagation();
                navigate(`/hotels/${rawId}`);
            };
            ...
            >
                ...
                >
                            ...
                            {items.map((it, i) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        />
                                        <div className="item-title">{it.alt}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    >
                        ...
                        <div
                            ref={frameRef}
                            className="viewer-frame h-full aspect-square flex max-w-[60vw] max-h-[60vh]"
